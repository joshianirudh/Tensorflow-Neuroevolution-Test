...
class WineEnvironment(BaseEnvironment):
    """
    ...
    def __init__(self, weight_training, config=None, verbosity=0, **kwargs):
        ...
        print("Setting up Boston environment...")
        wine_data = load_wine()
        x = wine_data['data']
        y = wine_data['target']
        x_train, x_test , y_train, y_test = train_test_split(x, y, test_size=0.2)
        (self.train_images, self.train_labels), (self.test_images, test_labels) = (x_train, y_train), (x_test, y_test)
        ...
        if not weight_training:
            raise NotImplementedError("Boston environment is being set up as non-weight training, though non-weight "
                                      "training evaluation not yet implemented for CIFAR10 environment")

        elif config is None and len(kwargs) == 0:
            raise RuntimeError("Boston environment is being set up as weight training, though neither config file nor "
    ...
    def duplicate(self) -> WineEnvironment:
        """
        @return: New instance of the XOR environment with identical parameters
        """
        if hasattr(self, 'epochs'):
            return WineEnvironment(True, verbosity=self.verbosity, epochs=self.epochs, batch_size=self.batch_size)
        else:
            return WineEnvironment(False, verbosity=self.verbosity)
